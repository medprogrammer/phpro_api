	...
	function setType($type){
		...
		switch($type){
			case UPLOAD_TYPE_IMAGE:
			  ...
			  $this->allowedMIMETypes=['image/gif','image/jpeg','image/png','image/svg+xml','image/tiff','image/vnd.microsoft.icon',' 	image/bmp','mage/x-windows-bmp'];
			  break;
			case '':
			  break;
		}
	...
	function process(){
		...
		if(!empty($this->allowedMIMETypes)){
			if(!in_array($file_type,$this->allowedMIMETypes)){
					$this->error=$file_name." : mime type not allowed.";
					return false;
			}
		}
